<script setup lang="ts">
import { useFormField } from "../../../../../utils/composable";
import {
  formSchema,
  selectedIndex,
} from "../../../../../utils/default-form-elements";
import { Input } from "../../../../../../ui/input";
import { computed, ref } from "vue";
import ValCard from "./ValCard.vue";

const selectedField = computed(() => formSchema.value[selectedIndex.value]);

const { updateParameterizedValidation, hasParameterizedValidation } =
  useFormField(selectedField, selectedIndex, formSchema);

const minimum = ref("");
const isActive = ref(hasParameterizedValidation("min"));

function updateMinValue() {
  if (isActive.value) {
    const minVal = `${minimum.value}`;
    updateParameterizedValidation("min", minVal, true);
  }
}

function toggleMinimum(newVal: boolean) {
  isActive.value = newVal;
  const minVal = `${minimum.value}`;
  updateParameterizedValidation("min", minVal, newVal);
}
</script>

<template>
  <ValCard
    :isActive="isActive"
    @update:isActive="toggleMinimum"
    label="Minimum"
    tooltip="Value must be greater or equal to the given value"
    :show-switch="true"
  >
    <Input
      v-show="isActive"
      v-model="minimum"
      @input="updateMinValue"
      @change="updateMinValue"
      placeholder="0"
      class="h-7 rounded-md px-2 py-1 text-[10px]"
      style="font-size: 10px"
    />
  </ValCard>
</template>
